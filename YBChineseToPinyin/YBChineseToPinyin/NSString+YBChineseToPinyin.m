...
NSString *YB_FindLetter(int nCode) {
    NSString *strValue = @"";
    switch(nCode) {
        case 6325:
        case 6436:
...
#define YB_HANZI_START 19968
#define YB_HANZI_COUNT 20902

static char yb_firstLetterArray[YB_HANZI_COUNT] =
...
char yb_pinyinFirstLet(unsigned short hanzi) {
    int index = hanzi - YB_HANZI_START;
    if (index >= 0 && index <= YB_HANZI_COUNT) {
        return yb_firstLetterArray[index];
    } else {
        return '#';
    }
}

@implementation NSString (YBChineseToPinyin)
...
- (NSString *)yb_pinyinParseWithSpace:(BOOL)space
{
    ...
    for (int i = 0; i < iLen; i++) {
        ...
        NSString *strRes = YB_FindLetter(nCode);
        if (space) {
            strValue = [strValue stringByAppendingString:[NSString stringWithFormat:@" %@",strRes]];
        } else {
    ...
    NSString *returnString = [[NSString alloc] initWithString:strValue];
    //去掉两端空格
    ...
    if( !string || 0 == [string length] )
        ...
        if(([string characterAtIndex:0] > 64 && [string characterAtIndex:0] < 91) ||
           ([string characterAtIndex:0] > 96 && [string characterAtIndex:0] < 123) ) {
            ...
            cLetter = yb_pinyinFirstLet((unsigned short)[string characterAtIndex:0]);
        
        if(cLetter > 95)
            cLetter -= 32;
    }	
    return [NSString stringWithFormat:@"%c",cLetter];
}

#pragma mark - 汉字转首字母简拼，小写输出
- (NSString *)yb_letterPinyinFromChinese
{
    NSArray *strArray = [[self yb_pinyinParseWithSpace:YES]componentsSeparatedByString:@" "];
